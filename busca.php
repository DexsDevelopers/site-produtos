...
<style>
    ...
    @keyframes glow {
        ...
        to {
            text-shadow: 0 0 30px rgba(255, 0, 0, 0.6), 0 0 40px rgba(255, 0, 0, 0.3);
        }
    }
...
<div class="busca-container">
    ...
    if (!empty($termo) || $categoria_id > 0 || $preco_min > 0 || $preco_max > 0 || $mostrar_todos) {
        try {
            ...
            if (!empty($termo)) {
                // Busca Avançada: Divide o termo em palavras para encontrar resultados mais específicos
                $termos = explode(' ', $termo_limpo);
                $termos = array_filter($termos, function ($t) {
                    return strlen($t) > 1; }); // Ignora letras soltas
    
                if (empty($termos)) {
                    $termos = [$termo_limpo];
                }

                // Agrupa condições para cada palavra (Lógica AND: o produto tem que ter TODAS as palavras, em qualquer campo)
                $word_conditions = [];
                ...
                if (!empty($word_conditions)) {
                    ...
                    // Fallback para busca exata se não houver palavras válidas
                    $where_conditions[] = "(LOWER(p.nome) LIKE LOWER(?))";
                    $params[] = $termo_busca;
                }
            }
            ...
            $termos = explode(' ', $termo_limpo);
            $termos = array_filter($termos, function($t) { return strlen($t) > 1; });
                ...
                <form method="GET" class="space-y-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 items-end">
                        ...
                        <div class="flex-1">
                            ...
                            <select name="ordenar"
                                class="w-full bg-brand-gray/50 border border-brand-gray-light text-white rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-brand-red">
                                <option value="relevancia" <?= $ordenar == 'relevancia' ? 'selected' : '' ?>>Relevância
                                </option>
                                <option value="nome" <?= $ordenar == 'nome' ? 'selected' : '' ?>>Nome A-Z</option>
                                <option value="preco_asc" <?= $ordenar == 'preco_asc' ? 'selected' : '' ?>>Menor Preço
                                </option>
                                <option value="preco_desc" <?= $ordenar == 'preco_desc' ? 'selected' : '' ?>>Maior Preço
                                </option>
                            </select>
                        </div>
            ...
            <div class="mt-10">
                                ...
                                <div class="produto-card rounded-xl overflow-hidden transition-all duration-300">
                                    ...
                                    <div class="p-4">
                                        <h3 class="font-bold text-lg text-white truncate">
                                            <?= htmlspecialchars($produto['nome']) ?></h3>
                                        <p class="text-sm text-brand-gray-text mt-1 truncate">
                                            <?= htmlspecialchars($produto['descricao_curta']) ?></p>
                                        <?php if (!empty($produto['categoria_nome'])): ?>
                                            <p class="text-xs text-brand-red mt-1">
                                                <?= htmlspecialchars($produto['categoria_nome']) ?></p>
                                        <?php endif; ?>
                                        <p class="text-xl preco-produto font-bold mt-3"><?= formatarPreco($produto['preco']) ?>
                                        </p>
                                    </div>
                                </div>
                            </a>
                ...
                <?php elseif (!empty($termo) || $categoria_id > 0 || $preco_min > 0 || $preco_max > 0): ?>
                    <div class="text-center py-16 bg-brand-gray/50 rounded-lg">
                        ...
                        <a href="busca.php"
                            class="inline-block mt-4 bg-brand-red hover:bg-brand-red-dark text-white font-bold py-2 px-6 rounded-lg transition-colors">
                            Limpar Filtros
                        </a>
                    </div>
                <?php endif; ?>

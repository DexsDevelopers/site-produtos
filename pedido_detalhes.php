...
<div class="w-full max-w-4xl mx-auto py-24 px-4">
    <div class="pt-16">
        <a href="minha_conta.php" class="text-brand-red hover:underline mb-6 inline-block">&larr; Voltar para Meus Pedidos</a>
        <h1 class="text-4xl md:text-5xl font-black text-white">Detalhes do Pedido #<?= htmlspecialchars($pedido['id']) ?></h1>
        ...
        <div class="mt-8 bg-brand-gray/50 p-6 rounded-xl ring-1 ring-white/10 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
                ...
                <p class="font-semibold text-white"><?= date('d/m/Y', strtotime($pedido['data_pedido'])) ?></p>
            </div>
            <div>
                <p class="text-sm text-brand-gray-text">Valor Total</p>
                <p class="font-semibold text-brand-red"><?= formatarPreco($pedido['valor_total']) ?></p>
            </div>
            <div>
            ...
            <div class="space-y-4">
                <?php foreach ($itens_pedido as $item): ?>
                    <div class="flex items-center bg-brand-gray/50 p-4 rounded-lg">
                        <img src="<?= htmlspecialchars($item['imagem'] ?? 'assets/images/placeholder.png') ?>" alt="<?= htmlspecialchars($item['nome_produto']) ?>" class="w-24 h-24 object-cover rounded-md mr-6">
                        <div class="flex-grow">
                            <h3 class="text-lg font-bold text-white"><?= htmlspecialchars($item['nome_produto']) ?></h3>
                            <p class="text-brand-gray-text">Quantidade: <?= $item['quantidade'] ?></p>
                            <p class="text-brand-gray-text">Preço Unitário: <?= formatarPreco($item['preco_unitario']) ?></p>
                        </div>
                        <div class="text-right">
                            <p class="text-lg font-bold text-white"><?= formatarPreco($item['preco_unitario'] * $item['quantidade']) ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>

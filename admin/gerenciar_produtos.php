    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="admin-card p-4 flex items-center gap-4 bg-white/5 border border-white/5 rounded-2xl">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-white"><?php echo $total_produtos; ?></p>
            </div>
        </div>
        <div class="admin-card p-4 flex items-center gap-4 bg-white/5 border border-white/5 rounded-2xl">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-white"><?php echo $total_destaques; ?></p>
            </div>
        </div>
    </div>
...
endforeach; ?>
            ...
            <select onchange="window.location.href = atualizarParametroUrl('ordem', this.value)"
                ...
                <option value="antigo" <?php echo($ordem == "antigo" ? "selected" : ""); ?>>Mais Antigos</option>
                <option value="preco_alto" <?php echo($ordem == "preco_alto" ? "selected" : ""); ?>>Preço (Maior)</option>
                <option value="preco_baixo" <?php echo($ordem == "preco_baixo" ? "selected" : ""); ?>>Preço (Menor)</option>
            </select>
                ...
                <div class="p-4 space-y-4">
                    <div class="flex gap-4">
                        ...
                        <div class="h-20 w-20 rounded-2xl bg-admin-gray-800 border border-white/10 overflow-hidden flex-shrink-0">
                            <?php if (!empty($produto["imagem"])): ?>
                            <img class="h-full w-full object-cover" src="../<?php echo htmlspecialchars($produto["imagem"]); ?>" alt="">
                            <?php
        else: ?>
                            <div class="h-full w-full flex items-center justify-center text-admin-gray-700"><i class="fas fa-image"></i></div>
                            <?php
                        ...
                        <div class="flex-1 min-w-0 flex flex-col justify-center">
                            <h3 class="text-white font-bold text-sm leading-tight mb-1"><?php echo htmlspecialchars($produto["nome"]); ?></h3>
                            <p class="text-[10px] text-admin-gray-500 mb-2 uppercase tracking-tight"><?php echo htmlspecialchars(isset($produto["categoria_nome"]) ? $produto["categoria_nome"] : "Sem Categoria"); ?></p>
                            <div class="flex items-center gap-2">
                                <span class="text-white font-black text-base"><?php echo formatarPreco($produto["preco"]); ?></span>
                                <?php if (isset($produto["frete_gratis"]) && $produto["frete_gratis"]): ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var bulkExtraFields = document.getElementById('bulk_extra_fields');
    ...
    if (bulkActionSelect) {
        bulkActionSelect.addEventListener('change', function() {
            ...
            if (action === 'change_category') {
                ...
                <?php foreach ($categorias as $cat): ?>
                o += '<option value="<?php echo $cat["id"]; ?>"><?php echo htmlspecialchars(addslashes($cat["nome"])); ?></option>';
                <?php
endforeach; ?>
                select.innerHTML = o;
